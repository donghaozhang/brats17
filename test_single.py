...
def test(config_file):
	...
	log('image_num is '.format(image_num))
	...
	for i in range(image_num):
		...
		t0 = time.time()
		log('temp_imgs is {} {}'.format(temp_imgs[0].shape, temp_imgs[1].shape))
		...
		log('the shape of pred1 is '.format(pred1.shape))
		pred1 = pred1 * temp_weight
		log('the shape of pred1 is '.format(pred1.shape))
		out_label = np.asarray(pred1, np.int16)
		# test_time.append(time.time() - t0)
		final_label = np.zeros(temp_size, np.int16)
		log('temp_size is '.format(temp_size))
		log('the value of temp_bbox '.format(temp_bbox))
		final_label = set_ND_volume_roi_with_bounding_box_range(final_label, temp_bbox[0], temp_bbox[1], out_label)
		log('final_label is '.format(final_label.shape))
		log('final label element :'.format(np.unique(final_label)))
		log('debug mode is '.format(img_names[0]))
